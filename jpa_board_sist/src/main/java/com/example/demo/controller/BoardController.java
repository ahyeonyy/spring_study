...
public class BoardController {
	...
	@GetMapping("/board/list/{pageNUM}/{writer}")
	public String list(Model model, 
		...
		//로그인한 회원의 정보를 갖고 오기 위하여
		//Authentication 객체를 생성합니다.
		Authentication authentication
		= SecurityContextHolder.getContext().getAuthentication();
		
		//Authentication 객체를 통해서 로그인한 회원의 
		//정보를 갖고 옵니다
		User user=(User) authentication.getPrincipal();
		
		//user를 통해서 로그인한 회원의 아이디를 갖고 옵니다.
		String loginId = user.getUsername();
		
		//아이디만 상태유지 할 수도 있고
		//dao를 통해서 회원객체를 갖고와 그것을 상태유지 할 수도 있어요
		...
		if(writer.equals("all")) {
			...
			totalRecord = bs.getTotalRecord(writer);
		}
		...
		if(writer.equals("all")) {
			...
			model.addAttribute("list", bs.findAll(start, end, writer));
		}
